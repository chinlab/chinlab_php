jQuery(document).ready(function($) {
		...
		$.ajax({
			type:"post",
			url:local+"/torder_indexJson.php",
			async:true,
...
//											}
		...
		$.ajax({
			type:"post",
			url:local+"/torder_OrderTypeAllJson.php",
			async:true,
			success:function(data){
				var str = "";
				var str2 = "";
			...
			$(document).on("click", ".edittrinfo",
				function() {
					...
					layerindex = layer.open({
						...
						content: "/chunfeng_oldorderreturnuserinfo.php?id=" + trid,
						end: function() {
							reflashrecord(trid);
							$(document.body).css({
								"overflow-x": "auto",
				...
				function reflashrecord(id) {
					$.post(local+"/torder_viewJson.php", {
							"id": id
						},
						function(result) {
							//console.log(result)
							var result = result.data[0][0]
							//console.log(result)
