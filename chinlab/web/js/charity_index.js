jQuery(document).ready(function($) {
		...
		$.ajax({
			type:"post",
			url:local+"/order_GetorderlistJson.php?order_type=13",
			async:true,
			...
			$(document).on("click", ".edittrinfo",
				function() {
					...
					layerindex = layer.open({
						...
						content: "/chunfeng_charityreturnuserinfo.php?order_id=" + trid,
						cancel: function(){ 
//		                   alert(1)
				            $.ajax({
				            	type:"post",
				...
				function reflashrecord(id) {
					$.post(local+"/order_viewJson.php", {
						...
						function(result) {
							...
							$tds.eq(5).text(result.order_state_desc);
