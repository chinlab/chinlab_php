...
//  });
//}
//function uploadEvent_bannerimg(){
//	$("#tcicon").on("fileuploaded", function (event, data, previewId, index) {
//  	console.log(data)
//  	console.log(data.response.data.url)
//      var code = data.response.state;	        
...
//      if (code == "0") {
//     		layer.msg(data.response.message, {icon: 1, closeBtn: 1, shadeClose: true,time:1000});
//     		productInfoImg.push(data.response.data.url);
////          afreshInit(4,30);
////          uploadEvent_bannerimg();
//          showimglist()
//      } else {
...
//      $("#tciconsrc").val("");
//  });
//  $('#tcicon').on('filebatchuploadsuccess', function(event, data, previewId, index) {
...
$(document).ready(function(){
	...
	$(document).on("click","#addTag",function(e){
	 	...
	 	if(taginput.length>5){
	 		layer.msg("超出5个字，添加失败", {icon: 5, closeBtn: 1, shadeClose: true,time:2000});
	 		$("#project_trait").val("");
	 		return false;
	 	}else{
	 		if(!reg.test(taginput)){
				...
				if(tagarr.length>5){
					layer.msg("超出6个不可再添加", {icon: 5, closeBtn: 1, shadeClose: true,time:2000});
				}else{
	...
	$(document).on("click", "#save",function(){
		...
		$.ajax({
			type:"post",
			url:local+"/shop/addoverseasgoods.php",
			data:{
