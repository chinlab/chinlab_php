jQuery(document).ready(function($){
	//分页
		$.ajax({
			type:"post",
			url:local+"/inquiry_indexJson.php",
			async:true,
...
//											}
//									}
									if(data[i].inquiry_state==1){
										state="未回复"
									}else if(data[i].inquiry_state==2){
										state="已回复"
									}else if(data[i].inquiry_state==3){
										state="已取消"
									}
									str += "<tr><td>"+ data[i].inquiry_time +"</td><td>"+ data[i].inquiry_name +"</td><td>"+ data[i].disease_name +"</td><td>"+ state +"</td><td><a class='edittrinfo' id='" + data[i].inquiry_id + "' trid='" + data[i].inquiry_id + "' href='javascript:void(0)'>查看回复</a>&nbsp;&nbsp;<a class='delrecord' id='" + data[i].inquiry_id + "del' trid='" + data[i].inquiry_id + "' href='javascript:void(0)'>删除</a></td></tr>" ;
								};
