...
class WxPayApi
{
    ...
    public static function unifiedOrder($inputObj, $timeOut = 6)
    {
        ...
        if (!$inputObj->IsNotify_urlSet()) {
            $inputObj->SetNotify_url(Yii::$app->weipay->config['NOTIFY_URL']);//异步通知url
        }

        if (true) {
            $inputObj->SetAppid(Yii::$app->weipay->config['APPID']);//公众账号ID
            $inputObj->SetMch_id(Yii::$app->weipay->config['MCHID']);//商户号
        } else {
            $inputObj->SetAppid(Yii::$app->weipay->config['GONGZONGHAO']);//公众账号ID
            $inputObj->SetMch_id(Yii::$app->weipay->config['GONGZONGHAO_MATCHID']);//商户号
        }
    ...
    {
        //如果不需要上报数据
        if (Yii::$app->weipay->config['REPORT_LEVENL'] == 0) {
            return;
        }
        //如果仅失败上报
        if (Yii::$app->weipay->config['REPORT_LEVENL'] == 1 &&
    ...
    private static function postXmlCurl($xml, $url, $useCert = false, $second = 30)
    {
        ...
        if ($useCert == true) {
            ...
            $options[CURLOPT_SSLCERT] = Yii::$app->weipay->config['SSLCERT_PATH'];
            $options[CURLOPT_SSLKEYTYPE] = 'PEM';
            $options[CURLOPT_SSLKEY] = Yii::$app->weipay->config['SSLKEY_PATH'];
        }
        ...
        if (!$result) {
            throw new \Exception("curl出错 weichatpay");
        }
        return $result;
    }
