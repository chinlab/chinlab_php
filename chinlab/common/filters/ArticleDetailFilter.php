 ...
 */
class ArticleDetailFilter extends ActionFilter
{
    public $enabled = true;
    ...
    public function beforeAction($action)
    {
        ...
        $userInfo = Yii::$app->getModule('patient')->runAction('usercache/getCacheBySession', ['session'=>$session_key]);
        if (!$userInfo) {
            $response->data = UResponse::formatData(UResponse::$code['InvalidSessionKey'], "登录会话失败");
            return true;
        }
        Yii::$app->runData->set("userInfo", $userInfo);
        return true;
    }
}